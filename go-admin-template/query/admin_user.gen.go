...
func newAdminUserModel(db *gorm.DB, opts ...gen.DOOption) adminUserModel {
	...
	_adminUserModel.Avatar = field.NewString(tableName, "avatar")
...
func (a adminUserModelDo) GetUserInfo(id int) (result types.UserInfoResponse) {
	...
	generateSQL.WriteString("SELECT u.id, u.username, u.realname, u.phone, u.avatar, r.NAME AS rolename, ( SELECT GROUP_CONCAT(`key`) FROM admin_auth WHERE JSON_CONTAINS( r.auth, JSON_ARRAY(id))) as authkeys FROM admin_user u LEFT JOIN admin_role r ON u.role_id = r.id WHERE u.id = ? ")
